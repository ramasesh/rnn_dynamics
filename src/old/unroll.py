...
def unroll_rnn(initial_states, input_sequences, rnn_update, readout=identity):
  ...
  function unrolls application of the RNN along the sequence. The RNN state is
  updated using the `rnn_update` function, and the `readout` is used to convert
  the RNN state to outputs (defaults to the identity function).
  ...
  Args:
    ...
    rnn_update: updates teh RNN hidden state, given (inputs, current state)
